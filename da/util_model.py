...
def model_lightgbm_kfold(df, colname=None, num_folds=2, stratified = False, colexclude=None, debug= False):
    ...
    regs = []
    # k-fold
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(df[colname],
                                                                df['is_match'])):
...
def sk_model_eval_classification(clf, istrain=1,
...
def sk_model_eval_classification_cv(clf,  X, y, test_size=0.5, ncv=1,method="random"):
    ...
    if method == "kfold" :
       ...
       for i,itrain, itest in enumerate(kf.split(X, y)):
          ...
          clf_list[i], _ = sk_model_eval_classification(clf, 1,
