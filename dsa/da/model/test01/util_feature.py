...
def pd_colnum_tocat(
    ...
    for c in colname:
        ...
        df[cbin] = df[cbin].apply(
            lambda x: x if x >= 0.0 else -
            1)  # 3 NA Values
...
def pd_num_correl_associations(
        df, colcat=None, mark_columns=False, theil_u=False, plot=True, return_results=False,
        **kwargs
):
    """
...
def pd_col_fillna(
    ...
    for col in colname:
        ...
        if method == "median_conditional":
            x = df.groupby(colgroupby)[col].transform(
...
def pd_fillna_emsvd(df, colname, k=None, tol=1E-3, maxiter=None):
    """
    ...
    return pd.DataFrame(Y_hat, columns=df.columns), mu_hat, U, s, Vt
...
def pd_col_fillna_datawig(
    ...
    if method == "datawig":
        ...
        for colna in colname_na:
            ...
            # Impute missing values and return original dataframe with
            # predictions
