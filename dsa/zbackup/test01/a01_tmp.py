...
profile.to_file(output_file= folder_out + "pandas_report_output.html")
...
dfnum, colnum_binmap = util_feature.pd_colnum_tocat(df, colname=colnum, colexclude=None, colbinmap=None,
...
dfnum_hot, colnum_onehot = util_feature.pd_col_to_onehot(dfnum[colnum_bin], colname=colnum_bin,
...
dftext = util_text.pd_coltext_nlp_stemmer( dftext, coltext, 
                                  lang="en", sep=" ", method="porterstemmer")
        
dftext.head(6)


#### Not Good
dftext0 = pd_coltext_nlp_stemmer( dftext0, coltext, 
                                  lang="english", sep=" ", method="stemmer" )
...
pipe_preprocess_coltext01 =[ 
  ...
  ,( util_text.pd_coltext_nlp_stemmer, { "coltext" : [col], 
...
for col in  coldate :
    dfdate_list[col] = util_date.pd_datestring_split( df , col, fmt="auto", return_val= "split" )
...
pipe_preprocess_coltext02 =[ 
           ( util_date.pd_coldate_split , {"colname": col, "fmt": "auto", "return_val": "split"  },  )        
...
# In[552]:
##### COnsider date as string and hash the into sub-space (good for dirtry date)
dfdate =  util_text.pd_coltext_fillna(df[coldate], coldate, "")


dfdate_hash, coldate_hash_model= util_text.pd_coltext_minhash(dfdate, coldate, n_component=[4, 2], 
...
# In[ ]:
### For initial modelling, we prefer using Post Model, feature selection
